# FeyaGate + OpenClaw 小白安装配置教程

> 从零开始，让 OpenClaw AI 助手控制你的智能家居设备。
...
| OpenClaw | 已安装 OpenClaw（[官网](https://openclaw.ai)） |
...
## 第三步：将 FeyaGate 接入 OpenClaw

这是关键一步——让 OpenClaw 能发现并使用 FeyaGate 的智能家居控制能力。
...
```
FeyaGate skill installed for OpenClaw.
Please restart OpenClaw to load the skill.
```

这条命令做了什么：在 `~/.openclaw/skills/` 下创建了一个名为 `feyagate` 的快捷方式，指向 `~/.feyagate/`。OpenClaw 启动时会自动读取里面的技能描述文件。

**重要：装完后必须重启 OpenClaw，技能才会生效。**
...
这些平台需要通过 OpenClaw 对话来完成授权（因为流程稍复杂）。等第五步验证 OpenClaw 能用后，对 OpenClaw 说：
...
如果一切正常，OpenClaw 会返回你账号下绑定的设备列表，包括设备名称、类型、房间等信息。
...
### Q：OpenClaw 重启后说不认识 FeyaGate 的命令

确认 symlink 存在：

```bash
ls -la ~/.openclaw/skills/feyagate
```
...
pip install ...    →   feyagate setup    →   feyagate start
                                                   ↓
  对 OpenClaw 说        重启 OpenClaw         接入 OpenClaw
 "列出我的设备"   ←   restart OpenClaw  ←   feyagate install-openclaw
                                                   ↓
                                            登录智能家居平台
                                            feyagate auth
```

全部完成后，你就可以通过和 OpenClaw 对话来控制家里的智能设备了。
